...
$(function(){
	...
	function loadTable(){
		 ...
		 var columns=[];
		ajaxSubmit(ctx+"/db/listProcedureInfo.action",false,true,param,function(data){
			if(data.code == 200){
				var td=[];
				
				var cls=getColumnsSetting(data.data.columns);
				var cd=getColumnsData(data.data.records,cls);
				cls.push({resizable:true,field:"manage",title:"操作",width:200});
				$.each(cd,function(){
					this.manage="<div><a  href='#' class='text-linkbutton' data-manager='select'  data-tableName='"+this.name+"' >查询</a><a  href='#' class='text-linkbutton' data-tableName='"+this.name+"' data-manager='delete'>删除</a></div>"
				});
				$("#dbmTable").datagrid({
					fit:true,
					minimized:false,
